...
export default function GeneratorToggle() {
  ...
  return (
    <motion.div
      ...
      className="sticky top-20 z-40 flex justify-center mb-8"
    >
      <div className="relative bg-background/60 backdrop-blur-2xl border rounded-full p-2 shadow-xl">
        <motion.div
        ...
        >
          {generators.map((generator) => (
            <ToggleGroupItem
              key={generator.id}
              value={generator.id}
              className={`relative px-6 py-3 rounded-full transition-all duration-500 overflow-hidden ${
            ...
            >
              ...
              >
                <motion.div 
                  ...
                  transition={{ duration: 0.5 }}
                >
                  <generator.icon className={`h-4 w-4 transition-all duration-300 ${
                    currentGenerator === generator.id 
                      ? "text-white scale-110" 
                      : "text-white"
                  }`} />
                </motion.div>
                <span className="font-medium tracking-wide">{generator.title}</span>
              </motion.div>
            </ToggleGroupItem>
          ))}
        </ToggleGroup>
